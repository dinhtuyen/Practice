...
string imgfilename01 = datapath + "LogoNTT.JPG";
string imgfilename02 = datapath + "MainImg.JPG";
string readvideofilename = datapath + "Test_Logo_NTT_02.mp4";
int mainImage(void)
{
  // Declare Ipoints and other stuff
  IpVec ipts;
  IplImage *img= cvLoadImage(imgfilename01.c_str());
  ...
  cv::namedWindow("OpenSURF", CV_GUI_NORMAL);
  drawIpoints(img, ipts,3);

  // Display the result
  //showImage(img);
  cv::imshow("OpenSURF", cv::cvarrToMat(img));
  waitKey(0);
  return 0;
}
...
int mainVideo(void)
{
  ...
  // Create a window
  namedWindow("OpenSURF", CV_GUI_NORMAL);
  ...
  while( 1 )
  {
   ...
   // img = cvQueryFrame(capture);
	  img = cvCloneImage(&(IplImage)camimg);
    // Extract surf points
    surfDetDes(img, ipts, false, 5, 4, 2, 0.0004f);
	//surfDetDes(img, ipts, false, 5, 4, 2, 0.0004f);
    // Draw the detected points
    drawIpoints(img, ipts,1);

    // Draw the FPS figure
   drawFPS(img);
	/*cv::Mat img001 = cv::cvarrToMat(img, true);*/
    // Display the result
    imshow("OpenSURF", cv::cvarrToMat(img));
...
int mainMatch(void)
{
  ...
  surfDetDes(img, ref_ipts, false, 5, 4, 2, 0.0001f);
...
int mainStaticMatch()
{
	...
	string datapath = "d:/VienIndustry40/SourceCode/OPENCV/OPENCV_PRACTICE/"
		"Practice/Data/PR0005/";
	string imgfilename01 = datapath + "Logo.JPG";
	string imgfilename02 = datapath + "MainImg.JPG";


  IplImage *img1, *img2;
  img1 = cvLoadImage(imgfilename01.c_str());
  img2 = cvLoadImage(imgfilename02.c_str());
...
int mainMatch01(void)
{
	string datapath = "d:/VienIndustry40/SourceCode/OPENCV/OPENCV_PRACTICE/"
		"Practice/Data/PR0004/";
	string imgfilename = datapath + "Logo.JPG";
	// Initialise capture device
	CvCapture* capture = cvCaptureFromCAM(CV_CAP_ANY);
	if (!capture) error("No Capture");
...
int mainVideofile(void)
{
	...
	// Extract reference object Ipoints
	surfDetDes(img, ref_ipts, true, 5, 4, 3, 0.0004f);
	// Main capture loop
	while (Cap.read(capimg))
	{
		// Grab frame from the capture source
		// img = cvQueryFrame(capture);
		img = cvCloneImage(&(IplImage)capimg);
		// Extract surf points
		// Detect and describe interest points in the frame
		surfDetDes(img, ipts, true, 5, 4, 3, 0.0004f);
...
int main(void)
{
  ...
  if (PROCEDURE == 7) return mainVideofile();
}
