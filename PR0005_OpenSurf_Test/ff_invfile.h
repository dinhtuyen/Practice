...
public:
	...
	int querytopn(unsigned int * vw, int nr, int n, int * docnames, float * scores, int sort);
	int querytopn(unsigned int *vw, int nr, int n, int *docnames, float *scores, int sort, int *m_pSImgSrc, int *m_pSImgLoc, int *m_pSImgFrNum);


	// Computes IDF weights from current database.
	void computeidf(void);
	void computeidf_sub(int nFile, unsigned int *pDbVW, int *pStart);
